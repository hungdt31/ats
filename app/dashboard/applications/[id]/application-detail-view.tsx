...
import {
  useDashboardApplicationDetail,
  type ApplicationDetail,
  type ApplicationDetailInterviewer,
} from "@/hooks/use-dashboard-applications";
...
function DetailSkeleton() {
  return (
    <div className="flex flex-col gap-6" aria-busy="true">
      <span className="sr-only">Đang tải hồ sơ ứng viên...</span>
...
export function ApplicationDetailView({ applicationId }: Props) {
  ...
  const { data, isLoading, isError, error } =
    useDashboardApplicationDetail(applicationId);

  const refresh = () =>
    void queryClient.invalidateQueries({
      queryKey: queryKeys.dashboard.applications.detail(applicationId),
    });
        ...
        <Card className="max-w-md w-full border-border/80">
          <CardHeader>
            <CardTitle className="text-lg">
              {is404 ? "Không tìm thấy đơn" : "Không thể tải dữ liệu"}
            </CardTitle>
            <CardDescription>
              {is404
                ? "Đơn ứng tuyển không tồn tại hoặc đã bị xoá."
          ...
          <CardContent>
            <Button asChild variant="outline" className="rounded-xl w-full sm:w-auto">
              <Link href="/dashboard/applications">Quay lại danh sách</Link>
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  const { application, interviewers } = data;
  return (
    <ApplicationView
...
// ---------------------------------------------------------------------------
// Inner presenter (hoist to keep JSX readable)
// ---------------------------------------------------------------------------
...
function ApplicationView({
  ...
  const statusConfig =
    STATUS_OPTIONS.find((s) => s.value === application.status) ?? STATUS_OPTIONS[0];

  const interviewersForForm = interviewers.map((iv) => ({
    id: iv.id,
          ...
          <Tabs defaultValue="audit-history" className="w-full">
            <TabsList className="mb-4">
              <TabsTrigger value="audit-history">
                Lịch sử ({application.application_status_history.length})
              </TabsTrigger>
              <TabsTrigger value="interviews">
                Phỏng vấn ({application.interviews.length})
              </TabsTrigger>
              <TabsTrigger value="emails">
                Nhật ký email ({application.email_logs.length})
              </TabsTrigger>
            </TabsList>
                ...
                <CardHeader>
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                    <div>
                      ...
                      <CardDescription>
                        Nhật ký chi tiết các lần cập nhật đơn ứng tuyển.
                      </CardDescription>
                    </div>
                    <StatusUpdateDialog
                  ...
                  {application.application_status_history.length === 0 ? (
                    ...
                    <div className="relative pl-6 border-l border-border/60 ml-3 space-y-6">
                      {application.application_status_history.map((history) => {
                        const fromLabel =
                          STATUS_OPTIONS.find((o) => o.value === history.from_status)?.label ||
                          history.from_status ||
                          "—";
                        const toLabel =
                          STATUS_OPTIONS.find((o) => o.value === history.to_status)?.label ||
                          history.to_status;
                        return (
                          <div key={history.id} className="relative space-y-1">
                            <span className="absolute -left-[28px] top-1.5 flex size-2 rounded-full bg-primary" />
                            ...
                            <p className="text-xs text-muted-foreground">
                              Bởi:{" "}
                              {history.users?.fullName || history.users?.email || "Admin/HR"}
                            </p>
                            {history.note && (
                              <p className="text-xs text-foreground bg-muted/40 p-3 rounded-xl border border-border/30 whitespace-pre-wrap leading-relaxed">
                                {history.note}
                ...
                <CardHeader>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between sm:gap-4 gap-2">
                    <div>
                      ...
                      <CardDescription>
                        Theo dõi danh sách các buổi phỏng vấn đã lên lịch hoặc hoàn thành.
                      </CardDescription>
                    </div>
                    <Dialog>
                      ...
                      <DialogContent className="sm:max-w-xl">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Tạo buổi phỏng vấn mới và tự động gửi email thông báo.
                          </DialogDescription>
                        </DialogHeader>
                        <div className="pt-2">
                          <CreateInterviewForm
                            applicationId={application.id}
                            interviewers={interviewersForForm}
                            onSuccess={onRefresh}
                          />
                        </div>
                      </DialogContent>
                            ...
                            <CardDescription className="text-xs">
                              ...
                              {iv.type === "video"
                                ...
                                : iv.type === "onsite"
                                  ? "Trực tiếp (Onsite)"
                                  : "Gọi điện / Khác"}{" "}
                              · Thời lượng: {iv.duration_minutes} phút
                            </CardDescription>
                          </CardHeader>
                          <CardContent className="space-y-3 text-xs">
                            ...
                            {iv.meeting_link && (
                              <div className="space-y-1 pt-1">
                                <span className="text-muted-foreground block">Link tham gia:</span>
                                <a
                            ...
                            {iv.location && (
                              <div className="space-y-1">
                                <span className="text-muted-foreground block">Địa điểm:</span>
                                <span className="font-medium text-foreground">{iv.location}</span>
                              </div>
                            )}
                            {iv.notes && (
                              <div className="space-y-1 pt-1">
                                <span className="text-muted-foreground block">Ghi chú từ HR:</span>
                            ...
                            {iv.interview_scores.length > 0 && (
                              <div className="mt-4 pt-3 border-t border-border/40 space-y-3">
                                ...
                                {iv.interview_scores.map((score) => (
                                  <div
                                    key={score.id}
                                    className="p-3 bg-background rounded-xl border border-border/30 space-y-2"
                                  >
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                      {[
                                        { label: "Điểm chung", value: score.overall_score },
                                        { label: "Kỹ thuật", value: score.technical_score },
                                        { label: "Giao tiếp", value: score.communication_score },
                                        { label: "Cultural Fit", value: score.cultural_fit_score },
                                      ].map(({ label, value }) => (
                                        <div key={label}>
                                    ...
                                    {score.feedback && (
                                      <p className="text-xs bg-muted/40 p-2 rounded-xl border border-border/20">
                                        Phản hồi: {score.feedback}
                                      </p>
                                    )}
                                  </div>
                                ))}
                            ...
                            <div className="mt-4 pt-3 border-t border-border/40 flex justify-end">
                              <Link
                                ...
                                className="inline-flex h-8 items-center justify-center rounded-2xl border border-input/60 bg-background px-3 text-xs font-medium hover:bg-muted transition-all"
                              >
                ...
                <CardHeader>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between sm:gap-4 gap-2">
                    <div>
                      ...
                      <CardDescription>
                        Danh sách các email hệ thống đã gửi cho ứng viên.
                      </CardDescription>
                    </div>
                    <Dialog>
                      <DialogTrigger asChild>
